...
async function seed() {
  ...
  for (const [name, brand, category, unit, basePrices] of PRODUCTS) {
    const { rows } = await client.query(
      'INSERT INTO products (name, brand, category, unit) VALUES ($1, $2, $3, $4) RETURNING id',
    ...
    for (let i = 0; i < slugOrder.length; i++) {
      ...
      if (price > 0) {
        ...
        if (smId) {
          await client.query(
            'INSERT INTO prices (product_id, supermarket_id, price) VALUES ($1, $2, $3)',
            [productId, smId, price]
          );
          priceCount++;
        }
      }
